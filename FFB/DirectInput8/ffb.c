 ...
 * - enumerates all DI joystick with FF support
 * - allows to select one of the DI joysticks
 * - enumerates its axes supporting FF
 * - enumerates its supported effects types and parameters
...
static struct {
    GUID guid;
    char * name;
    unsigned char hasSpring;
    unsigned char hasDamper;
} devices[MAX_DEVICES] = {};

static unsigned int nbDevices = 0;
...
static BOOL CALLBACK DI_DeviceObjectCallback(LPCDIDEVICEOBJECTINSTANCE dev, LPVOID pvRef)
{
    ...
    printf("Found axis with actuator: %s\n", name);
    if (axis == -1) {
        axis = offset;
    }
...
DI_EffectCallback(LPCDIEFFECTINFO pei, LPVOID pv)
{
    if (DI_GUIDIsSame(&pei->guid, &GUID_Spring)) {
        devices[deviceIndex].hasSpring = 1;
    } else if (DI_GUIDIsSame(&pei->guid, &GUID_Damper)) {
        devices[deviceIndex].hasDamper = 1;
    }
    unsigned int i;
...
static HWND createWindow() {
    ...
    return CreateWindow(wce.lpszClassName, "DI test app", WS_POPUP | WS_VISIBLE | WS_SYSMENU, cursor_pos.x,
            cursor_pos.y, 1, 1, NULL, NULL, hInstance, NULL);
}
...
int openHapticDevice(unsigned int index) {

    HRESULT ret = IDirectInput8_CreateDevice(dinput, &devices[deviceIndex].guid, &device, NULL);
    if (FAILED(ret)) {
        fprintf(stderr, "IDirectInput8_CreateDevice failed\n");
        return DIENUM_CONTINUE;
    }

    ret = IDirectInputDevice8_SetCooperativeLevel(device, raw_hwnd, DISCL_EXCLUSIVE | DISCL_BACKGROUND);
    if (FAILED(ret)) {
        fprintf(stderr, "IDirectInput8_CreateDevice failed\n");
        return -1;
    }
    ...
    DIDEVCAPS deviceCaps = { .dwSize = sizeof(DIDEVCAPS) };

    ret = IDirectInputDevice8_GetCapabilities(device, &deviceCaps);
    if (FAILED(ret)) {
        fprintf(stderr, "IDirectInputDevice8_GetCapabilities failed\n");
    }

    if (deviceCaps.dwFlags & DIDC_FORCEFEEDBACK) {

        ret = IDirectInputDevice8_SendForceFeedbackCommand(device, DISFFC_RESET);
        if (FAILED(ret)) {
            fprintf(stderr, "IDirectInputDevice8_SendForceFeedbackCommand failed (DISFFC_RESET)\n");
        }

        ret = IDirectInputDevice8_SendForceFeedbackCommand(device, DISFFC_SETACTUATORSON);
        if (FAILED(ret)) {
            fprintf(stderr, "IDirectInputDevice8_SendForceFeedbackCommand failed (DISFFC_SETACTUATORSON)\n");
        }
    }
    ...
    if (axis == -1) {
        printf("No axis has force-feedback support\n");
        return -1;
    }
...
static BOOL CALLBACK EnumHapticsCallback(const DIDEVICEINSTANCE * pdidInstance, VOID * pContext)
{
    ...
    devices[nbDevices].guid = pdidInstance->guidInstance;
    devices[nbDevices].name = strdup(pdidInstance->tszProductName); // in real apps this should probably be converted
    ++nbDevices;

    if (nbDevices == sizeof(devices) / sizeof(*devices)) {
...
int closeHapticDevice() {
    ...
    if (device != NULL) {
        HRESULT ret = IDirectInputDevice8_SendForceFeedbackCommand(device, DISFFC_SETACTUATORSOFF);
        if (FAILED(ret)) {
            fprintf(stderr, "IDirectInputDevice8_SendForceFeedbackCommand failed (DISFFC_SETACTUATORSOFF)\n");
        }
        ret = IDirectInputDevice8_SendForceFeedbackCommand(device, DISFFC_RESET);
        if (FAILED(ret)) {
...
int selectHapticDevice() {
    ...
    printf("Available haptic devices:\n");
    ...
    printf("Enter the haptic device number: ");
    fflush(stdout);

    unsigned int choice = UINT_MAX;
    if (scanf("%d", &choice) != 1 || choice >= nbDevices) {
...
int init() {
    ...
    if (raw_hwnd == NULL) {
        ...
        LPTSTR pBuffer = NULL;
        if (!FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, error, 0,
    ...
    ret = IDirectInput8_EnumDevices(dinput, DI8DEVCLASS_ALL, EnumHapticsCallback, NULL,
...
int main(int argc, char * argv[]) {
    ...
    deviceIndex = selectHapticDevice();
    if (deviceIndex < 0) {
        return -1;
    }

    if (openHapticDevice(deviceIndex) < 0) {
        return -1;
    }

    if (devices[deviceIndex].hasSpring) {
    ...
    if (devices[deviceIndex].hasDamper) {
